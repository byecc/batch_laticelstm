...
class BatchGazCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, use_bias=True):
        ...
        super(BatchGazCell, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.use_bias = use_bias
        self.weight_ih = nn.Parameter(
    ...
    def forward(self, input_, hx,c_input,offset):
        """

        :param input_: batch_size*word_emb_dim
        :param c_input:c_num*hidden_dim
        :param hx:
        :return:
        """
        h_0, c_0 = hx
        ...
        if c_input is None:
            ...
            c_num = c_input.size(0)
            max_num = int(c_num/batch_size)
            emb_size = input_.size(1)
            offset_ten = autograd.Variable(torch.LongTensor(offset),requires_grad = False)
            emb_list = [input_[i].unsqueeze(0) for i in range(batch_size) for j in range(max_num)]
            input_ = torch.cat(emb_list)
            # alpha_wi = torch.addmm(self.alpha_bias, input_, self.alpha_weight_ih).expand(c_num, self.hidden_size)
            alpha_wi = torch.addmm(self.alpha_bias, input_, self.alpha_weight_ih)
            alpha_wh = torch.mm(c_input, self.alpha_weight_hh)
            word_i= torch.sigmoid(alpha_wi + alpha_wh)  #word 'i' gate
            word_i = word_i.view(-1,self.hidden_size)
            temp = torch.autograd.Variable(torch.zeros(c_num,self.hidden_size))
            # print(c_num,offset)
            word_i = temp.index_copy_(0,offset_ten,word_i[offset])
            word_i = word_i.view(batch_size,-1,self.hidden_size)
            i = i.unsqueeze(1)
            # i = i.unsqueeze(1).expand(batch_size,max_num,self.hidden_size)
            # i = i.contiguous().view(-1,self.hidden_size)
            # temp = torch.autograd.Variable(torch.zeros(c_num,self.hidden_size))
            # i = temp.index_copy_(0, offset_ten, i[offset])
            # word_i = word_i.view(batch_size,-1,self.hidden_size)
            # i = i.view(batch_size,-1,self.hidden_size)

            alpha = torch.exp(torch.cat([i, word_i], 1))
            alpha_sum = alpha.sum(1)
            ## alpha = softmax for each hidden element
            alpha = alpha.view(-1,batch_size*self.hidden_size)
            alpha_sum = alpha_sum.view(-1)
            alpha = torch.div(alpha, alpha_sum)
            alpha = alpha.view(batch_size,-1,self.hidden_size)
            # g_list = [g[i].unsqueeze(0) for i in range(batch_size) for j in range(int(c_num/batch_size))]
            # g = torch.cat(g_list)
            # temp = torch.autograd.Variable(torch.zeros(c_num, self.hidden_size))
            # g = temp.index_copy_(0,offset_ten,g[offset])
            # g = g.view(batch_size,-1,self.hidden_size)
            g = g.unsqueeze(1)
            c_input = c_input.view(batch_size,-1,self.hidden_size)
            merge_i_c = torch.cat([g, c_input], 1)
            c_1 = merge_i_c * alpha
            c_1 = c_1.sum(1).squeeze(1)
            h_1 = o * torch.tanh(c_1)
        return h_1, c_1
...
class BatchLatticeLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, word_alphabet_size, word_emb_dim, pretrain_word_emb=None,
                 fix_word_emb=True, direction=False):
        super(BatchLatticeLSTM, self).__init__()
        self.word_emb = nn.Embedding(word_alphabet_size, word_emb_dim)
        if pretrain_word_emb is not None:
            self.word_emb.weight.data.copy_(torch.from_numpy(pretrain_word_emb))
        else:
        ...
        self.rnn = BatchGazCell(input_dim, hidden_dim)
        self.word_rnn = WordLSTMCell(word_emb_dim, hidden_dim)
        self.hidden_dim = hidden_dim
    ...
    def forward(self, emb, gaz_input, hidden=None):
        ...
        gaz, gaz_offset = [], []
        w_hx, w_cx = None, None
        input_c = [[[] for j in range(seq_len)] for i in range(batch_size) ]
        for idx in range(seq_len):
            if h is None and c is None:
                h = autograd.Variable(torch.zeros(batch_size, self.hidden_dim))
                c = autograd.Variable(torch.zeros(batch_size, self.hidden_dim))
            input_emb = emb[:,idx,:]
            current_c = [item[idx] for item in input_c]
            max_len = max([len(item) for item in current_c])
            if max_len>0:
                w_cx = autograd.Variable(torch.zeros(max_len*batch_size,self.hidden_dim))
                for ix,item in enumerate(current_c):
                    for iy,cc in enumerate(item):
                        w_cx[ix*max_len+iy] = cc
                        gaz_offset.append(ix*max_len+iy)
                # print("max_len:",max_len)
                # w_cx = w_cx.view(batch_size,max_len,self.hidden_dim)
            else:
                w_cx = None
            if max_len>1:
                pass
                # print('1')
            hx, cx = self.rnn(input_emb, (h, c), w_cx, gaz_offset)
            h, c = hx, cx
            h_out.append(h.unsqueeze(1))
            c_out.append(c.unsqueeze(1))
            gaz, gaz_offset = [], []
            w_hx,w_cx = None,None
            item_len = [len(item[idx][0]) for item in gaz_input if item[idx]]
            if item_len:
                max_c_num = max(item_len)
                for ix,g in enumerate(gaz_input):
                    if g[idx]:
                        for iy,item in enumerate(g[idx][0]):
                            gaz.append(item)
                    gaz.extend([0 for i in range(max_c_num*(ix+1)-len(gaz))])
                if len(gaz)!=max_c_num*batch_size:
                    for ix,g in enumerate(gaz_input):
                        print(g[idx])
                # assert(len(gaz)==max_c_num*batch_size)
                word_var = autograd.Variable(torch.LongTensor(gaz),requires_grad=False)
                word_emb = self.word_emb(word_var)
                # word_emb = self.word_dropout(word_emb)
                hx_list = [hx[i].unsqueeze(0) for i in range(batch_size) for j in range(max_c_num)]
                cx_list = [cx[i].unsqueeze(0) for i in range(batch_size) for j in range(max_c_num)]
                w_cx = self.word_rnn(word_emb, (torch.cat(hx_list), torch.cat(cx_list)))
                for m,g in enumerate(gaz_input):
                    if g[idx]:
                        for n,item in enumerate(g[idx][1]):
                            input_c[m][idx+item-1].append(w_cx[m*max_c_num+n,:])
                pass
        return torch.cat(h_out,1),torch.cat(c_out,1)
